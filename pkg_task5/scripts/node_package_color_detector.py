...
def detect_pkgs_of_clr(image, hsv_image, pkg_color):
    ''' 
        ...
        Return : A list containing bounding box coordinates of all the detected packages of
                a particular colour.
    '''
    ...
    if pkg_color == "red":
        ...
        #creating a mask to detect green coloured packages
        mask = cv2.inRange(hsv_image, lower_green, upper_green)
    elif pkg_color == "yellow":
        lower_yellow = np.array([20, 10, 10])
        upper_yellow = np.array([30, 255, 255])
        #creating a mask to detect yellow coloured packages
        mask = cv2.inRange(hsv_image, lower_yellow, upper_yellow)
     # Finding all the contours in a masked image
    ...
    for cnt in contours: 
        ...
        if (len(approx) == 4):
            ...
            #Comparing ratio to actual height to width ratio of a square to  filter out
            #rectangle shaped contours
            if (ar >= 0.95 and ar <= 1.05):
              #Appending  bounding box coordinates of detected packages to bounding_boxes list
...
def identify_pkgs(bounding_boxes, pkg_color):
    '''
    A function to identify the name of detected packages of a particular colour using the
    co-ordinates of bounding boxes.
...
def detect_pkgs_color(shelf_image):
    '''
        detect_pkgs_color() is a function that detects the colour of all packages placed on
        the shelf.
    ...
    for clr in pkg_colors:
        #Calling detect_pkgs_of_clr() to detect packages of a particular colour
        detect_pkgs_of_clr(resized_image, hsv_image, clr) 

def main():

    rospy.init_node('node_package_color_detector', anonymous=True)
